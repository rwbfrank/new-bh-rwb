...
if ( ! class_exists( 'rsssl_importer' ) ) {
  ...
  */
  ...
  /*
      load_and_save: if true, changes are safed to DB

  */

  public function remove_from_scan_array($url, $post_id=false, $load_and_save = true){
    if ($load_and_save) rsssl_scan::this()->load_results();
    ...
    */
    ...
    if ($load_and_save) rsssl_scan::this()->save_results();
...
public function ignore_url(){
       ...
       $response = json_encode( array( 'success' => false, 'error' => $error ) );
         ...
         error_log("The new array");
         error_log(print_r(rsssl_scan::this()->ignored_urls, true));
